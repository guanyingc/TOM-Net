...
function M.parse(arg)
    ...
    cmd:option('-crop_h',         384,                    'Data: crop height')
    cmd:option('-crop_w',         384,                    'Data: crop width')
    ...
    ------------ Network Options ----------------------
    cmd:option('-refine',         true,        'Flag for the refine stage' )
    cmd:option('-coarse_net',     '',          'Network: CoarseNet')
    cmd:option('-networkType',    'RefineNet', 'Network: network ')
    cmd:option('-layers_num',       12,        'Network: layer number')
    cmd:option('-channel_sz',       64,        'Network: channel number in resblock')
    cmd:option('-filter_sz',        3,         'Network: kernel size')
    cmd:option('-down_num',         3,         'Network: downsample factor')
    cmd:option('-ms_num',           4,         'Network: scales level for CoarseNet')
    ----------- Checkpoint options ---------------
    ...
    -------------- Loss Options -----------
    cmd:option('-flow_w',          1,          'Loss: Flow weight')
    cmd:option('-rho_w',           1,          'Loss: Attenuation mask weightRho weight')
    ------------- Display Options -------------
...
function M.getSaveDirName(opt)
    ...
    local dirRoot = paths.dirname(paths.dirname(opt.coarse_net))
    local fileName = str_utils.splitext(paths.basename(opt.coarse_net))
    local dName = string.format('%s_%s_%s', opt.date, opt.prefix, fileName) .. opt.networkType

    local params = {'scale_h', 'crop_h'}
    for k, v in pairs(params) do
        dName = dName .. string.format('_%s-%d', v, opt[v]) 
    end
    local params = {'flow_w', 'rho_w'}
    for k, v in pairs(params) do
    ...
    local logDir = paths.concat(dirRoot, 'refinement', dName, 'logdir')
    local save   = paths.concat(dirRoot, 'refinement', dName, 'checkpointdir')
    return logDir, save, dName
end

function M.checkpath(opt)
    if opt.coarse_net == '' or not paths.filep(opt.coarse_net) then
        print(opt.coarse_net)
        error("Please specify the path of trained coarse_net or file not exists")
    end
